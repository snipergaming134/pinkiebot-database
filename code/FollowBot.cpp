...
// User settings
CatVar bot(CV_SWITCH, "fb_bot", "0", "Master Followbot Switch",
           "Set to 1 in followbots' configs");
CatVar follow_distance(CV_FLOAT, "fb_distance", "175", "Follow Distance",
                       "How close the bots should stay to the target");
CatVar follow_activation(CV_FLOAT, "fb_activation", "175",
                         "Activation Distance",
                         "How close a player should be until the followbot "
                         "will pick them as a target");
CatVar mimic_slot(
    CV_SWITCH, "fb_mimic_slot", "0", "Mimic selected weapon",
    "If enabled, this bot will select same weapon slot as the owner");
CatVar always_medigun(CV_SWITCH, "fb_always_medigun", "0", "Always use Medigun",
                      "Medics will always use Medigun");
CatVar crumb_draw(CV_SWITCH, "fb_crumb_draw", "1", "Draw Crumbs",
                  "Draws the path made for the followbot");
CatVar roaming(CV_SWITCH, "fb_roaming", "0", "Roaming",
               "Allows the bot to find a different target if it cant find one "
               "using the steam id");
CatVar sync_taunt(CV_SWITCH, "fb_sync_taunt", "0", "Mimic taunts",
                  "Bots will taunt if target is taunting");
...
void AfterCreateMove()
{
    ...
    while (it != selection.end())
    {
        ...
        {
#if ENABLE_VISUALS == 1
            hacks::shared::esp::AddEntityString(entity, "[SELECTED]",
                                                colors::orange);
    ...
    while (it != selection_secondary.end())
    {
        ...
        else
        {
            ...
            hacks::shared::esp::AddEntityString(
                entity, "[SELECTED (SECONDARY)]", colors::orange);
            if (fmod(g_GlobalVars->curtime, 2.0f) < 1.0f)
            {
                hacks::shared::esp::SetEntityColor(entity, colors::yellow);
            }
...
void DoWalking()
{
    ...
    for (int i = 1; i < HIGHEST_ENTITY; i++)
    {
        ...
        if (ent->m_bEnemy)
            continue;
        if (g_pLocalPlayer->v_Origin.DistTo(ent->m_vecOrigin) > 3400.0F)
            continue;
    ...
    if (roaming && CE_BAD(best_target))
    {
        ...
        {
            if (g_pLocalPlayer->v_Origin.DistTo(target_last->m_vecOrigin) <
                    3400.0F &&
                target_last->m_bAlivePlayer && !crumbStopped)
            {
        ...
        if (CE_BAD(best_target) ||
            g_GlobalVars->curtime - 2.5F > crumb_prune_timeout)
        {
			...
			for (int i = 0; i < HIGHEST_ENTITY; i++) {
				...
				if (g_pLocalPlayer->v_Origin.DistTo(ent->m_vecOrigin)
						> (float) follow_activation)
					continue;
				if (!ent->IsVisible())
					continue;
				...
				float scr = 4096.0f
						- g_pLocalPlayer->v_Origin.DistTo(ent->m_vecOrigin);
...
#if ENABLE_VISUALS == 1
		hacks::shared::esp::AddEntityString(found_entity, "[FOLLOWING]",
				colors::green);
    ...
    // If curtime is less than the last time we checked the slot, then reset our
    // check timer
    if (g_GlobalVars->curtime < last_slot_check)
        last_slot_check = 0.0f;

    // If we have a follow target, user settings tell us that we should mess
    // with the slot, the local player is alive, and our target is alive
    if (following_idx2 && (always_medigun || mimic_slot) &&
        (g_GlobalVars->curtime - last_slot_check > 1.0f) &&
        !g_pLocalPlayer->life_state &&
        !CE_BYTE(found_entity, netvar.iLifeState))
    {
        ...
        int owner_weapon_eid =
            (CE_INT(found_entity, netvar.hActiveWeapon) & 0xFFF);
    ...
    // Switch to different types of following mechanisms depending on the type
    // we need to go to
    switch (current_follow_type)
    {
    case EFollowType::VECTOR: // If were using a vector to follow, we just go
                              // directly to it

        // If destination_point_time is more than curtime than we reset it to
        // zero
        if (destination_point_time > g_GlobalVars->curtime)
            destination_point_time = 0.0f;

        // If we havent reached our destination and the destination point
        // timeout isnt more than 5 seconds, then we continue to walk to the
        // destination point
        if (g_GlobalVars->curtime - destination_point_time < 5.0f)
        {
            ...
            // If we have reached the destination point then we want to disable
            // the vector followbot
    ...
    case EFollowType::ENTITY: // If were using a player to follow, we use the
                              // breadcrumb followbot

        if (CE_GOOD(found_entity))
        {
            // If the bot is lost but it finds the player again, start the
            // followbot again.
            if (crumbStopped)
            {
                crumbForceMove = true;
        ...
        if (!crumbStopped)
        {
            ...
            // Generate new breadcrumbs made by the player only if they are
            // close to the ground. If the bot is told to generate a starting
            // point, it does that as well.
            if ((found_entity->m_vecOrigin.DistTo(breadcrumbs[crumbTop]) >
                     40.0F ||
                 crumbFindNew) &&
                DistanceToGround(found_entity) < 40)
            {

                // Add to the crumb.
                CrumbTopAdd(found_entity->m_vecOrigin);

                // If the bot was forced to select a point, we tell it that we
                // no longer need a new one and clear the bottom crumb to use
                // the newest one
                if (crumbFindNew)
                {
                    crumbFindNew = false;
            ...
            // Prune used crumbs from the stack to make way for new ones when
            // you get close to them.
            if (g_pLocalPlayer->v_Origin.DistTo(breadcrumbs[crumbBottom]) <
                40.0F)
            {
                ...
                // When the bot is forced to move to the player, since they have
                // reached their destination we reset the var
                crumbForceMove = false;

                // Check 15 times for close crumbs to prune, this allows simple
                // miss steps to be smoothed out as well as make room for new
                // crumbs
                for (int i = 0; i < 15; i++)
                {

                    // When one is close or too high, just bump the array and
                    // reset the stuck timer
                    if (g_pLocalPlayer->v_Origin.DistTo(
                            breadcrumbs[crumbBottom]) < 60.0F &&
                        crumbArrayLength > 1)
                    {
                        CrumbBottomAdd();
            ...
            // When player to follow is too far away. the bot cant see the
            // player or the bot is forced to the player, then follow
            // breadcrumbs if movement is allowed
            if ((g_pLocalPlayer->v_Origin.DistTo(found_entity->m_vecOrigin) >
                     (float) follow_distance ||
                 crumbForceMove) &&
                crumbArrayLength >= 1 && allow_moving)
            {
                followbot::WalkTo(breadcrumbs[crumbBottom]);

                // If a crumb hasnt been pruned in a while, it probably cant
                // travel to it so reset and wait for the player to collect it.
                ...
                // If the bot is next to the player then we clear our crumbs as
                // theres no need to follow previously generated ones.
            }
            else if (g_pLocalPlayer->v_Origin.DistTo(
                         found_entity->m_vecOrigin) < 100.0F &&
                     found_entity->IsVisible())
            {
                CrumbReset();
                crumbForceMove = false;
            }
    ...
    if (following_idx2)
    {
        ...
        // If follow target is too far away and the local player is zoomed, then
        // unzoom
        ...
        if (HasCondition<TFCond_Zoomed>(found_entity))
        {
            ...
            // If the local player isnt zoomed and its class is sniper, then
            // zoom in to mimic the followed target
...
CatCommand follow("fb_follow", "Follows you (or player with SteamID specified)",
                  [](const CCommand &args) {

                      // Set our target steam id to the argument put into the
                      // command
...
CatCommand follow_me("fb_follow_me", "Makes all bots follow you", []() {
    ...
    if (ipc::peer)
    {
        ...
        ipc::peer->SendMessage((const char *) &id, 0,
                               ipc::commands::set_follow_steamid, 0, 0);
    }
});
...
CatCommand tool("fb_tool", "Followbot multitool", [](const CCommand &args) {
    ...
    else if (!strcmp(args.Arg(1), "move"))
    {
        ...
        // Send the vector through the ipc server and tell the bots to move to
        // it
        float array[3] = { destination.x, destination.y, destination.z };
        ipc::peer->SendMessage((const char *) array, MakeMask(),
                               ipc::commands::move_to_vector, nullptr, 0);
    ...
    else if (!strcmp(args.Arg(1), "stay"))
    {
        ...
        // Send a message through the ipc server to notify the bots to stop
        // moving
        ipc::peer->SendMessage(nullptr, MakeMask(), ipc::commands::stop_moving,
                               nullptr, 0);
    ...
    else if (!strcmp(args.Arg(1), "follow"))
    {
        ...
        // Send a message through the ipc server to notify the bots to start
        // moving
        ipc::peer->SendMessage(nullptr, MakeMask(), ipc::commands::start_moving,
                               nullptr, 0);
    }
});
...
void SelectEntity(int idx)
{
    ...
    std::set<int> &current_selection =
        IsBot(entity) ? selection : selection_secondary;
...
// Ipc message handlers. When an ipc event comes from an ipc server, its called
// below
void AddMessageHandlers(ipc::peer_t *peer)
{

    // When the ipc server gives us a steam id to follow, we run the following
    peer->SetCommandHandler(ipc::commands::set_follow_steamid,
                            [](cat_ipc::command_s &command, void *payload) {
                                // Log in console the steam id that were
                                // following
                                logging::Info("IPC Message: now following %ld",
                                              *(unsigned *) &command.cmd_data);
    ...
    peer->SetCommandHandler(
        ipc::commands::move_to_vector,
        [](cat_ipc::command_s &command, void *payload) {
            ...
            logging::Info("IPC Message: moving to %.2f %.2f %.2f", data[0],
                          data[1], data[2]);
    ...
    peer->SetCommandHandler(ipc::commands::start_moving,
                            [](cat_ipc::command_s &command, void *payload) {
                                allow_moving = true;
                            });

    // When the ipc server tells us to stop moving, we run the following
    peer->SetCommandHandler(ipc::commands::stop_moving,
                            [](cat_ipc::command_s &command, void *payload) {
                                allow_moving = false;
                            });
}
...
void WalkTo(const Vector &vector)
{
    ...
    if (CE_VECTOR(LOCAL_E, netvar.vVelocity).IsZero(1.0f))
    {
        ...
        if (!idle_time)
            idle_time = g_GlobalVars->curtime;
...
void CrumbReset()
{
    ...
    // We just reset the bookkeeping vars for the array, no need to clear the
    // array as everything will be re-written anyways
    crumbTop            = 0;
    crumbBottom         = 0;
    crumbArrayLength    = 0;
    crumb_prune_timeout = g_GlobalVars->curtime;
...
void CrumbTopAdd(Vector crumbToAdd)
{
    ...
    // The array can only hold so many crumbs, once it goes over its cap, stop
    // the bot to prevent un-needed movement
...
void CrumbBottomAdd()
{
    ...
    // A check to detect if too many crumbs have been removed. Without crumbs
    // the bot will just use random variables in the array. To prevent
    // un-nessasary movement, just clear the array and wait for player
...
// A Function for when we want to draw out the crumbs in the array onto the
// screen
void DrawFollowbot()
{
    ...
    /*switch (current_follow_type) {
    case EFollowType::VECTOR: // If our follow type is a vector, then we just
    draw a rect on the vector

        // Create a vector for the screen, run world to screen to bring the
    world vector into the screen vector, then draw a rect at the point Vector
    scn; draw::WorldToScreen(destination_point, scn); draw_api::FilledRect(scn.x
    - 3, scn.y - 3, 6, 6);
        // Return as we are finished
        break;

    case EFollowType::ENTITY: // If our follow type is entity, then we draw out
    the crumbs here logging::Info("Drawcrumb1");
        ...
        for (int i = 0; i < crumbArrayLength; i++) {
            ...
            // We need 2 crumbs to draw a line, so we get the number in the
    array for 2 crumbs tmpCrumb1 = crumbBottom + i; tmpCrumb2 = crumbBottom + i
    + 1;
            ...
            draw_api::Line(scnSrt.x, scnSrt.y, scnEnd.x - scnSrt.x, scnEnd.y -
    scnSrt.y, colors::white);
    ...
    for (int i = 0; i < crumbArrayLength; i++)
    {
        ...
        // We need 2 crumbs to draw a line, so we get the number in the array
        // for 2 crumbs
        ...
        // Draw a line from one crumb to the other
        draw_api::draw_line(scnSrt.x, scnSrt.y, scnEnd.x - scnSrt.x,
                            scnEnd.y - scnSrt.y, colors::white, 0.5f);

        // If this is our first iteration, draw a box on 1
        if (i == 0)
            draw_api::draw_rect(scnSrt.x - 3, scnSrt.y - 3, 6, 6,
                                colors::white);
    }
    return;
}
